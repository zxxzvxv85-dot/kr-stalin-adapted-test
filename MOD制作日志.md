...
- 军事改革物资不再显示在决议分类标题和顶部说明中。28个消耗物资的手动军改国策均通过统一条件提示显示当前库存，全部委员会自动国策也在自动推进提示中显示库存；复用原版“工厂与扳手”素材并注册为专用 `GFX_RUS_fr_military_reform_materials` 文本图标，物资获得、变动、变量名称和国策提示统一使用该图标。
