...
for(const widget of gui.filter(x=>['instantTextBoxType','buttonType','iconType'].includes(x.key))){
 ...
 const width=widget.key==='instantTextBoxType'?+get(b,'maxWidth'):widget.key==='buttonType'?(sprite.includes('naval_')?26:123):sprite.includes('crop_')?32:24;
 const height=widget.key==='instantTextBoxType'?+get(b,'maxHeight'):widget.key==='buttonType'?34:width;
...
for(const lang of ['simp_chinese','english','russian']){
 ...
 function expand(t,depth=0){if(depth>5)return '';return t.replace(/\[(FRA|ENG)\.GetName\]/g,(_,tag)=>countryNames[tag==='FRA'?0:1]).replace(/\$([\w.]+)\$/g,(_,k)=>expand(loc.get(k)||k,depth+1)).replace(/\[\?([^|]+)\|(\d)\]/g,(_,key,d)=>key.includes('produced')?'99999':key.includes('income')?'99000':key.includes('stock')?'4500.0':d==='0'?'100':d==='1'?'100.0':'1.25').replace(/\[([^\]]+)\]/g,(_,name)=>{const block=defined.find(n=>get(n.value,'name')===name);if(!block)return '';return block.value.filter(n=>n.key==='text').map(n=>expand(loc.get(get(n.value,'localization_key'))||'',depth+1)).sort((a,b)=>width(b)-width(a))[0]||'';});}
