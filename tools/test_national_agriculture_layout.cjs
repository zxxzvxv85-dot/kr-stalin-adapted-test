...
for(const lang of ['simp_chinese','english','russian']){
 ...
 function expand(t,depth=0){if(depth>5)return '';return t.replace(/\$([\w.]+)\$/g,(_,k)=>expand(loc.get(k)||k,depth+1)).replace(/\[\?([^|]+)\|(\d)\]/g,(_,key,d)=>key.includes('produced')?'99999':key.includes('income')?'99000':key.includes('stock')?'4500.0':d==='0'?'100':d==='1'?'100.0':'1.25').replace(/\[([^\]]+)\]/g,(_,name)=>{const block=defined.find(n=>get(n.value,'name')===name);if(!block)return '';return block.value.filter(n=>n.key==='text').map(n=>expand(loc.get(get(n.value,'localization_key'))||'',depth+1)).sort((a,b)=>width(b)-width(a))[0]||'';});}
