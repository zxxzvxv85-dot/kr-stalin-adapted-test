...
const chineseHelp = [
  ['农业生产',
    '£RUS_nat_text_food£ 每季免费生产配置额度为10，每种作物最多配置10。',
    '农机承诺成功、土改成功各永久增加生产配置额度+2，免费额度最多14。',
    '超过免费额度后，可继续点击作物加号扩产。单项最多10，合计最多50。自动配置仅使用免费额度。',
    '超额配置 E = 实际配置 - 免费额度（最低为0）',
    '下季消费品期望 = min(E, 5) × 0.5% + max(E - 5, 0) × 1%',
    '季末按最终配置确定扩产惩罚，下一季全程生效，每季替换。超额5时为+2.5%，超额10时为+7.5%。',
  ...
  ['农机生产',
    ...
    '£RUS_nat_text_stock£ 国内在役农机目标为400单位，土改成功后的下一季提高至500单位。农机仓库上限5000单位。',
    '每季在役农机损耗率随机为10%/15%/20%/25%。恶劣天气使损耗率额外§R+5%§!，最高30%。仓库农机不参与在役损耗。',
    '平均覆盖率按本季每日在役数量计算，使作物产量变为60%至110%，季末补机不能追溯改善整季收成。',
    '£RUS_nat_text_orders£ 作出承诺后540天内须完成推广拖拉机3次，且从农业系统启动起累计自产农机达到4000单位。',
  ...
  ['季度报告',
    ...
    '£RUS_nat_text_reform£ 乌斯季诺夫路线土改目标：200。土改期限：720天。成功后超额积分成为可消费余额，消费不降低改革阶段。',
    '失败后停止积分与兑换，农业继续运转。出口没有额外土改积分。',
    '£RUS_nat_text_food£ 年度精神依据内需平均满足率与年末储备评定。粮食内需满足率不足90%时惩罚加深一级，恢复至至少90%一季降低一级。甜菜缺口按比例降低每周稳定度，完全断供时每周-0.4%。纺织原料短缺时稳定度-6%、消费品期望+2.5%。恢复供给后解除。粮食与纺织惩罚可叠加。'],
];
