...
img('card_order_browser_background','order_scroll',10,403,3)
save('order_transparent',Image.new('RGBA',(1,1)))
g.append('containerWindowType = { name = "card_order_viewport" position = { x = 10 y = 403 } size = { width = 482 height = 156 } clipping = yes verticalScrollbar = "right_vertical_slider" smooth_scrolling = yes background = { spriteType = "GFX_RUS_card_order_transparent" } gridboxType = { name = "card_foreign_orders_grid" position = { x = 4 y = 4 } size = { width = 456 height = 100%% } slotsize = { width = 456 height = 1 } max_slots_horizontal = 1 add_horizontal = no } }')
txt('card_no_foreign_orders',label('no_foreign_orders',['本季暂无国外订单','No foreign orders this quarter','Нет внешних заказов в этом квартале']),28,453,430,40,p=3,extra='check_variable = { RUS_nat_order_category = 1 } check_variable = { RUS_nat_foreign_order_count = 0 }')
...
for j,(group,crop,names) in enumerate([('food','wheat',['粮食供给','Food supply','Продовольствие']),('beet','beet',['制糖原料','Sugar processing','Сырьё для сахара']),('textile','cotton',['纺织原料','Textile materials','Текстильное сырьё']),('machine',None,['国内农机需求','Domestic machinery','Техника для страны'])]):
 row_start=len(g);trigger_start=len(tr);y=0;img('card_domestic_'+group,'order_row',4,y,tip=tip)
 ...
 for line in g[row_start:]:
  leaf=re.search(r'name = "([^"]+)"',line).group(1)
  tr.append(f'{leaf}_visible = {{ check_variable = {{ RUS_nat_order_row = {100+j} }} }}')
g.append('}')
browser_entry='\n'.join(g);g=main_g
