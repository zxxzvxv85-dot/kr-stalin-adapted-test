...
for i,(buyer,tag,machine) in enumerate(rows):
 container=f'card_order_entry_{i}';visible(container,0);tr.append(f'{container}_visible = {{ check_variable = {{ RUS_nat_order_row = {i} }} }}')
 g.append(f'containerWindowType = {{ name = "{container}" size = {{ width = 456 height = 76 }} position = {{ x = 0 y = 0 }}')
