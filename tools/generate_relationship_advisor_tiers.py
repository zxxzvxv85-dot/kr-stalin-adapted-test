...
ADVISORS = [
    ...
    },
    {
        ...
        "bop": 0.01,
        "legacy": ["RUS_relationship_scaled_kirov", "KR_prince_of_petrograd", "KR_bolshevik_kirov_bop"],
    },
    {
        "group": "psr",
        "slug": "mayorov",
...
def render_traits() -> str:
    ...
    for advisor in ADVISORS:
        ...
        for tier in GROUP_TIERS[advisor["group"]]:
            ...
            if "bop" in advisor:
                lines.append(f"\t\tpower_balance_weekly = {number(advisor['bop'])}")
            lines.extend(["\t}", ""])
    lines.append("}")
    return "\n".join(lines) + "\n"
...
def render_legacy_cleanup() -> list[str]:
    ...
    for advisor in ADVISORS:
        ...
        if identity:
            # A character may already have the identity trait in a leader or
            # second-in-command role while it is missing from the advisor role.
            lines.append(
...
def render_effects() -> str:
    ...
    lines.extend([
        ...
        "\tif = {",
        "\t\tlimit = { NOT = { has_country_flag = RUS_native_relationship_advisor_traits_migrated_v2 } }",
        "\t\tRUS_stalin_clear_legacy_relationship_advisor_traits = yes",
        "\t\tset_country_flag = RUS_native_relationship_advisor_traits_migrated",
        "\t\tset_country_flag = RUS_native_relationship_advisor_traits_migrated_v2",
        "\t}",
...
def render_localisation(language: str) -> str:
    ...
    for advisor in ADVISORS:
        for tier in GROUP_TIERS[advisor["group"]]:
            lines.append(f"  {trait_id(advisor, tier)}: \"${advisor['name']}$\"")
    return "\n".join(lines) + "\n"
