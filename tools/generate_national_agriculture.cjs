...
for(const amount of [5,10])effect('decision_score_'+amount,iff(mode,
 ...
 v('decision_award',100)+sub('decision_award',n('decision_score'))+cl('decision_award',0,amount)+
...
text('nat_decision_score',10,605,482,label(n('decision_score_line'),'决议累计供分：[?RUS_nat_decision_score|1]/100（含最初四项）','Decision points: [?RUS_nat_decision_score|1]/100 (includes initial four)','Очки решений: [?RUS_nat_decision_score|1]/100 (включая первые четыре)'),0,20);
...
scripted('GetRUSNatDecisionCap',[[ck('decision_score','>',99.999),n('decision_cap_full')]],n('decision_cap_open'));
for(const amount of [5,10])label(n('decision_score_'+amount+'_tt'),
 ...
 `Land reform points: up to §G+${amount}§!.\\nCumulative decision points: [?RUS_nat_decision_score|1]/§Y100§! (including 40 from the initial four).\\n[GetRUSNatDecisionCap]`,
